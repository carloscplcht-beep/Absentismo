...
export const buildExecutiveNarrative = (
  ...
  const yearText = forecast.targetYear ? ` del año ${forecast.targetYear}` : "";
  ...
  return `${filtersText}, se registran ${formatNumber(metrics.ausenciasUnicas)} ausencias, que acumulan ${formatNumber(metrics.diasAusenciaHastaFinP)} dias de ausencia. De estos, ${formatNumber(metrics.diasSustituidosHastaFinP)} dias han sido sustituidos, lo que supone una cobertura por dias del ${formatPercent(metrics.porcentajeSustitucionDias)}. El coste total asociado a sustituciones asciende a ${formatCurrency(metrics.totalNomina)}. La prevision de cierre anual${yearText}, calculada con metodologia ${forecast.method}, estima ${formatNumber(forecast.central.diasAusenciaHastaFinP)} dias de ausencia y ${formatCurrency(forecast.central.totalNomina)} de coste para ese año. La tendencia mensual del absentismo es ${absenceTrend.direction.toLowerCase()}. Las categorias con mayor impacto gestor son ${top}. ${forecastQuality}`;
};
