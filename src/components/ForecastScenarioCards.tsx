...
export function ForecastScenarioCards({ forecast }: { forecast: ForecastResult }) {
  ...
  return (
    <div className="kpi-grid">
      <KPICard label={`Prevision dias ausencia${yearText}`} value={formatNumber(forecast.central.diasAusenciaHastaFinP)} detail={`Metodo: ${forecast.method}`} />
      <KPICard label={`Prevision coste total${yearText}`} value={formatCurrency(forecast.central.totalNomina, true)} detail={formatCurrency(forecast.central.totalNomina)} />
      <KPICard label={`Prevision dias no sustituidos${yearText}`} value={formatNumber(forecast.central.diasNoSustituidos)} tone={forecast.central.diasNoSustituidos ? "warn" : "good"} />
